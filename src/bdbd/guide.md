...
debt is gone, and what-ifs. People use it in a terminal. LLM agents and scripts use **agent
mode**: add `--agent` to any command and it prints exactly one JSON object on stdout.

```
bdbd [--agent] [--select PATHS] [--db PATH] [--no-tidy] COMMAND [ARGS]
```

Global flags work anywhere on the line. `--select` implies `--agent`. `BDBD_AGENT=1` turns
agent mode on for every run. Never do budget arithmetic yourself: ask bdbd and report its
numbers.
...
- Failure: `{"ok": false, "command": "...", "error": {"code": "...", "message": "...",
  "hint": "..."}}`. Exit codes: 0 ok, 1 a problem with the request (unknown flow, bad date),
  2 a usage mistake (missing argument, unknown option).
...
- `--select a,b.c,rows[0].date,series[-1].balance` keeps only those paths of `data`. When a path
  misses, the error lists the keys that are there: fix the path from it.
- Agent mode never prompts. Anything a person would be asked for must be passed as arguments.
...
bdbd remembers the balance you give it: `bdbd balance 3200` (optionally `--on DATE`). It is
cash on hand **before that day's scheduled items**; if the day's items have already posted, say
`--posted` (agent mode assumes `--pending` and warns when that day has items). Every projection
starts from the latest balance, carried forward day by day through the budget, so a balance
from Tuesday still works on Friday. Outputs report where the starting balance came from:
...
### Where things stand
- `bdbd` → `today`, `balance` (the opening block for today), `spare_balance`, `spare
  ...
  rows until the next income, at least 10 days), `debts {…}` (as `bdbd debts`).
- `bdbd balance [AMOUNT] [--on DATE] [--posted|--pending] [--forget]` → without AMOUNT: `today`
  ...
  the previous balance), `difference_from_expected`.
- `bdbd upcoming [--days N | --until DATE] [--balance A] [--weekly-spend A]` → `from`, `until`,
  `opening`, `weekly_spend`, `items[{date, name, kind, amount, balance_after, debt?}]`,
  `lifestyle_total`, `ending_balance`. Ledger `kind`: income, expense, debt_payment,
  extra_payment, payoff, settle.
- `bdbd cal [MONTH]` (oct, 2026-11, next, +2) → `month`, `opening`, `days[{date, items[{name,
...
- `bdbd show FLOW` → the flow plus `upcoming[]` (next dates), `monthly`, and for debts
  ...
  total_interest_remaining}`. FLOW is a name (any case) or id.
...
- `bdbd edit FLOW [--name] [--amount] [--when TEXT] [--from D] [--until D | --no-until]
  ...
  → the updated flow.
- `bdbd rm FLOW [-y]` → `{removed: flow}` (its debt record and events go too).
...
- `bdbd tags` → `tags[{name, flows, flow_names[], expense_monthly, income_monthly}]`;
  `bdbd tags rename OLD NEW`; `bdbd tags rm TAG`.
...
- `bdbd debts` (or `bdbd debt`) → `debts[{flow, name, balance (today), annual_rate, payment,
  paid_off_on, interest_remaining, tags}]`, `total_owed`, `monthly_payments`,
  `interest_remaining`, `debt_free_on`.
...
- `bdbd debt schedule FLOW [--rows N | --all] [--as-of D] [--until D | --months N (600)]
  ...
  payment, interest, principal, balance, kind}]`, `rows_truncated`, `solved_payment`.
...
- `bdbd summary [--tag T] [--actual --months N] [--by tag|flow|both] [--all]` → `net
  ...
  occurrences_per_year, is_debt}]`, `by_tag[{tag, expense_monthly, …}]`, `untagged`,
...
- `bdbd plan --extra A [--strategy avalanche|snowball] [--order A,B] [--tag T] [--exclude D]
  ...
  `monthly_outlay {scheduled_payments, with_extra}`, `steps[{order, name, annual_rate,
...
- `bdbd config [KEY [VALUE]] [--unset]` → `{config, keys}` (or `{key, value}`). The only key:
  `weekly_spend`.
- `bdbd export [-o FILE]` → the backup `{bdbd_export: 1, schema_version, config, flows[],
  balances[], exported_at}`; `bdbd import FILE [--replace]` → `{imported_flows}`.
...
```
bdbd --agent project --until 2026-12-12 --select spare_balance,ending_balance,spare,opening
```
...
`bdbd --agent balance AMOUNT`.

**"What will I spend on the car by Dec 12?"** `bdbd --agent tags` (find the tag), then
`bdbd --agent spend car --until 2026-12-12 --select total,by_flow`.

**"What does the car cost a month?"** `bdbd --agent summary --tag car --select by_tag,by_flow`.

**"When can I book a $550 flight without dropping below $1,000?"**
```
bdbd --agent earliest --floor 1000 --add-expense "Flight:550@?" --select status,date,result
bdbd --agent project --on DATE --add-expense "Flight:550@?" --until +2m      # the detail
```

**"How fast can I clear my debts with $300 a month more?"**
`bdbd --agent plan --extra 300 --select debt_free_on,months,interest_saved,steps,cash_check`;
run it with `--strategy snowball` too when asked to compare.

**"If I sell the car for $15,000 on Oct 15, when am I better off?"**
`bdbd --agent breakeven --settle "Car loan:15000@2026-10-15" --stop-tag "car@2026-10-15"`.

**Keeping it current**: `add`, `edit`, `rm`, `pause`, `debt set`, `debt extra` and friends. After a
change, read it back with `bdbd --agent show NAME` and confirm it to the person.
...
`invalid_import`, `file_not_found`, `integrity`.
